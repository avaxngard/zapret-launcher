...
class VPNChecker:
    def __init__(self, callback=None):
        ...
        self.test_hosts = [('8.8.8.8', 53), ('1.1.1.1', 53), ('208.67.222.222', 53)]
        self.test_urls = ['https://www.google.com', 'https://www.cloudflare.com', 'https://www.microsoft.com']
        ...
        self.vpn_process_keywords = [
            ...
            'windscribe', 'vyprvpn', 'ivpn', 'airvpn', 'zenmate', 'hidester', 
            ...
            'anyconnect', 'forticlient', 'globalprotect', 'ipsec', 'ikev2', 
            'l2tp', 'pptp', 'sstp',
            
            'proxifier', 'proxycap', 'sockscap', 'widecap', 'proxychains',
            'v2rayN', 'v2rayng', 'nekoray', 'nekobox', 'sing-box',
            'mihomo', 'hysteria.exe', 'tuic.exe', 'naive.exe',
            ...
            'tunnelbear.exe', 'windscribe.exe', 'cyberghost.exe',
            
            '--vpn', '--proxy', 'vpn_', '_vpn', 'vpn-', '-vpn',
            
            'openvpnserv', 'vpnui', 'vpnmgr', 'vpngui', 'openvpnas', 
            'ovpntray', 'ovpnadmin',
            
            'hide.me', 'hideme', 'privatevpn', 'ovpn.com', 'oceanvpn',
            'fastvpn', 'securevpn', 'supervpn', 'freevpn', 'betternet',
            'hotspot', 'protect', 'vpnify', 'vpnhub', 'vpnarea',
            'vpnproxy', 'zenvpn', 'vpnzone', 'surfeasy', 'ibvpn', 
            'leapvpn', 'ghostvpn', 'vpnbook', 'vpnexpress',
            'goosevpn', 'xvpn', 'vpnon', 'vpnoff',
            
            'vpn', 'proxy'
        ]
        
        self.vpn_interface_keywords = ['tun', 'utun', 'vpn', 'openvpn', 'wireguard']
    ...
    def check_internet_connection(self, timeout: int = 2) -> Tuple[bool, str]:
        ...
        try:
            result = subprocess.run(['ping', '-n', '1', '8.8.8.8'], capture_output=True, text=True, creationflags=subprocess.CREATE_NO_WINDOW, timeout=timeout)
        ...
        try:
            for proc in psutil.process_iter(['pid', 'name', 'exe', 'cmdline']):
                try:
                    ...
                    skip_processes = ['svchost.exe', 'textinput.exe', 'explorer.exe', 'taskhost.exe', 'dwm.exe', 'aggregatorhost.exe']
    ...
    def check_vpn_interfaces(self) -> Tuple[bool, List[str]]:
        ...
        try:
            if sys.platform == 'win32':
                result = subprocess.run(['ipconfig', '/all'], capture_output=True, text=True, creationflags=subprocess.CREATE_NO_WINDOW, timeout=5)
    ...
    def check_vpn_routes(self) -> bool:
        try:
            if sys.platform == 'win32':
                result = subprocess.run(['route', 'print'], capture_output=True, text=True, creationflags=subprocess.CREATE_NO_WINDOW, timeout=5)
    ...
    def check_external_ip(self) -> Optional[str]:
        try:
            ...
            for service in services:
                try:
                    req = urllib.request.Request(service, headers={'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64)'})
        ...
        def monitor_loop():
            while not self._stop_event.is_set():
                try:
                    ...
                    self._notify('vpn_status', 'VPN detected' if result['vpn_detected'] else 'No VPN detected', result)
                except Exception as e:
