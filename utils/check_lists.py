...
class ListChecker:
    CHECK_FILES = ["list-general.txt", "list-custom.txt", "list-google.txt", "list-white.txt"]
    ...
    def extract_domain(self, line: str) -> Tuple[str, bool]:
        line = line.strip()
        ...
        try:
            with open(file_path, 'r', encoding='utf-8') as f:
                for line_num, line in enumerate(f, 1):
                    domain, is_wildcard = self.extract_domain(line)
                    if domain:
                        if is_wildcard:
                            wildcard_domains.add(domain)
                        key = f"{domain}|{'wildcard' if is_wildcard else 'exact'}"
                        domains[key].append(line_num)
        except Exception:
            return {}, 0, 0
        
        duplicates = {}
        for key, lines in domains.items():
            if len(lines) > 1:
                domain = key.split('|')[0]
                duplicates[domain] = lines
    ...
    def check_all_files(self) -> Dict[str, Dict]:
        ...
        for filename in self.CHECK_FILES:
            ...
            results[filename] = {'duplicates': duplicates, 'total': total, 'unique': unique}
        return results
