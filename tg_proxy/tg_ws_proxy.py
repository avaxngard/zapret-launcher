...
from .logger_setup import get_log_file_path, setup_tg_logging
...
async def _read_client_init(reader, writer, secret, label, masking):
    ...
    if first_byte[0] == TLS_RECORD_HANDSHAKE and masking:
        ...
        log.debug(f"[{label}] ClientHello first 20 bytes: {client_hello[:20].hex()}")

        tls_result = verify_client_hello(client_hello, secret)

        if tls_result is None:
            log.debug("[%s] Fake TLS verify failed (size=%d rec=%d) -> masking",
...
def run_proxy(stop_event: Optional[asyncio.Event] = None):
    ...
    log = logging.getLogger('tg-mtproto-proxy')
    log.info("=" * 60)
    log.info("  Starting TG Proxy")
    log.info(f"  Host: {proxy_config.host}")
    log.info(f"  Port: {proxy_config.port}")
    log.info(f"  Secret: {proxy_config.secret[:8]}..." if len(proxy_config.secret) >= 8 else "  Secret: <invalid>")
    log.info(f"  Fake TLS: {proxy_config.fake_tls_domain or 'disabled'}")
    log.info(f"  DC redirects: {proxy_config.dc_redirects}")
    log.info("=" * 60)
...
async def stop_proxy(stop_event: asyncio.Event):
    if stop_event:
        stop_event.set()
